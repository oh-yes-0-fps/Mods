...
def GetDatatableValue(datatable: unrealsdk.UObject, row: str, column: str, pawn=GetPlayerController().Pawn) -> float:
    """
...
def GetDatatableRow(datatable: unrealsdk.UObject, row: str) -> float:
    """
    Gets row from a data table entry.
    """
    static = Get_cls_instance('GbxDataTableFunctionLibrary')
    handle = (datatable, row)
    out = static.GetDataTableValueFromHandle(handle, datatable.RowStruct, ())
    return out


def GetAttValue(att_obj: unrealsdk.UObject, pawn=GetPlayerController().Pawn, default: float = 0) -> float:
    """
    Returns the value of the attribute att_name for the player_bpchar.
    """
    statics = Get_cls_instance('GbxAttributeFunctionLibrary')
    att_value = statics.GetValueOfAttribute(att_obj, pawn, default)
    return att_value
...
def GetLocation() -> Tuple[float, float, float]:
    ...
    """
    location_vector = GetPlayerController().Pawn.CapsuleComponent.RelativeLocation
...
def enable_cheats() -> None:
    PC = unrealsdk.FindAll('OakPlayerController')[0]
    PC.CheatManager = unrealsdk.ConstructObject(
        Class=PC.CheatClass.Name, Outer=PC, Name='SDKCheatManager')
...
def __Datatable_test():
    ...
    dt_obj = GetObject(
        'DataTable', '/Game/Enemies/Skag/_Shared/_Design/Balance/Table_Skag_Balance_Unique.Table_Skag_Balance_Unique')
    row_name = 'TrialBoss'
...
def __Att_Value_Test():
    ...
    att_obj = GetObject(
        'GbxAttributeData', '/Game/GameData/Weapons/Att_Weapon_Damage.Att_Weapon_Damage')
